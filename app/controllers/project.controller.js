...
class ProjectController {
  ...
  async createProject(req, res, next) {
    try {
      ...
      if(!result) throw {status : 400, message : "Adding the project encountered a problem"}
  ...
  async removeProject(req, res, next) {
    try{
      ...
      if(deleteProjectResult.deletedCount == 0) throw {status : 400, message : "The project was not deleted"}
       return res.status(200).json({
         status : 200, 
         success : true,
  ...
  async updateProject(req, res, next) {
    try {
      ...
      if(updateResult.modifiedCount == 0) throw {status : 400, message : "The update was not successful"}
      return res.status(200).json({
        status : 200,
        success : true,
